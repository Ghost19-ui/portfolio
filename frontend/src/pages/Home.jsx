...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fallback default data if DB is empty or connection fails
        setData({
            profile: { role: 'RED TEAM OPERATOR' }, projects: [], certificates: []
        })
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div className="min-h-screen bg-cyber-black flex items-center justify-center text-cyber-red font-mono animate-pulse">&gt; INITIALIZING_SYSTEM...</div>;

  return (
    <div className="bg-cyber-black min-h-screen text-cyber-text selection:bg-cyber-red selection:text-white pb-32 font-sans">
      <NeuralBackground />
      ...
      <section id="home" className="min-h-screen flex flex-col justify-center items-center relative z-10 px-4 pt-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center">
           ...
           <h1 className="text-5xl md:text-8xl font-black font-mono tracking-tighter text-white mb-6 relative">
              <span className="relative z-10">TUSHAR<span className="text-cyber-red">SAINI</span></span>
              <span className="absolute top-0 left-0 -ml-1 text-red-600 opacity-50 animate-pulse blur-[1px]">TUSHARSAINI</span>
           </h1>
           <p className="text-cyber-muted font-mono max-w-xl mx-auto text-lg">
             <span className="text-cyber-red">&gt;</span> {profile.bio || "Cybersecurity enthusiast and Full Stack Developer specializing in offensive security tools."}
           </p>
        </motion.div>
        ...
        >
          {/* 1. PROFILE & CONTACT CARD (Moved Socials Here) */}
          <div className="md:col-span-2 md:row-span-2 bg-cyber-dark border border-white/10 rounded-2xl p-6 flex flex-col items-center text-center relative overflow-hidden group">
            ...
            {/* Photo */}
            <div className="w-24 h-24 rounded-full border-2 border-cyber-red p-1 mb-4 shadow-[0_0_30px_rgba(220,38,38,0.4)]">
              <img src="/profile.jpeg" alt="Profile" className="w-full h-full rounded-full object-cover" />
            </div>
            <h2 className="text-2xl font-bold font-mono text-white">TUSHAR SAINI</h2>
            <p className="text-cyber-red tracking-widest text-xs mt-1 mb-6 font-mono">{profile.role}</p>

            {/* Contact & Socials Below Photo */}
            <div className="w-full border-t border-white/10 pt-4 flex flex-col gap-3 text-sm font-mono text-gray-400">
                {profile.email && <div className="flex items-center justify-center gap-2"><Mail size={14} className="text-cyber-red"/> {profile.email}</div>}
                {profile.phone && <div className="flex items-center justify-center gap-2"><Phone size={14} className="text-cyber-red"/> {profile.phone}</div>}
                
                <div className="flex justify-center gap-4 mt-2">
                    {profile.github && <a href={profile.github} target="_blank" rel="noreferrer" className="hover:text-cyber-red transition-colors"><Github size={20}/></a>}
                    {profile.linkedin && <a href={profile.linkedin} target="_blank" rel="noreferrer" className="hover:text-cyber-red transition-colors"><Linkedin size={20}/></a>}
                    {profile.instagram && <a href={profile.instagram} target="_blank" rel="noreferrer" className="hover:text-cyber-red transition-colors"><Instagram size={20}/></a>}
                </div>
            </div>
          </div>

          {/* 2. LIVE RESUME TERMINAL (Replaces old social grid) */}
          <div className="md:col-span-2 md:row-span-1 bg-black border border-white/10 rounded-2xl p-4 font-mono text-xs relative overflow-hidden group cursor-pointer" onClick={() => setShowResumeModal(true)}>
             ...
             <div className="flex items-center justify-between text-cyber-muted mb-2 pb-2 border-b border-white/5">
                <div className="flex gap-2 items-center"><Terminal size={14} className="text-cyber-red" /> <span>LIVE_INTEL_FEED</span></div>
             ...
             <div className="space-y-1 text-gray-500">
                ...
                <p className="text-cyber-red animate-pulse">&gt; CLICK TO ACCESS CLASSIFIED DOCUMENT</p>
             </div>
             <FileText size={60} className="absolute bottom-4 right-4 text-cyber-red opacity-20 group-hover:opacity-40 transition-opacity group-hover:scale-110 duration-300"/>
          </div>

          {/* 3. SKILLS LIST */}
          <div className="md:col-span-2 md:row-span-1 bg-cyber-dark border border-white/10 rounded-2xl p-6 font-mono text-xs overflow-hidden">
             <div className="flex items-center gap-2 text-cyber-muted mb-4 pb-2 border-b border-white/5">
                <Cpu size={14} /> <span>SKILL_MATRIX</span>
             </div>
             <div className="grid grid-cols-2 gap-2">
               {["PEN-TESTING", "REACT / NODE", "PYTHON BASH", "BURP SUITE", "AD SECURITY", "MONGODB"].map((s,i) => (
                 <div key={i} className="bg-white/5 px-3 py-2 rounded border border-white/5 flex justify-between items-center">
                   <span className="text-gray-400">{s}</span>
                   <span className="text-cyber-red text-[10px]">[OK]</span>
                 </div>
               ))}
             </div>
          </div>
      ...
      <AnimatePresence>
        {showResumeModal && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 md:p-8">
                <div className="bg-cyber-dark border border-white/10 w-full max-w-4xl h-[80vh] rounded-xl relative flex flex-col">
                    <div className="flex justify-between items-center p-4 border-b border-white/10 bg-black/50 font-mono">
                        <h3 className="text-white flex items-center gap-2"><FileText size={16} className="text-cyber-red"/> CLASSIFIED_RESUME.pdf</h3>
                    ...
                    {profile.resumeUrl && (
                        <a href={profile.resumeUrl} download className="p-4 bg-cyber-red text-black text-center font-bold font-mono hover:bg-red-500 transition-colors">DOWNLOAD PDF</a>
                    )}
                </div>
            </motion.div>
        )}
      ...
      <section id="projects" className="py-20 px-4 max-w-6xl mx-auto">
         <div className="flex items-center gap-4 mb-12">
            ...
            <div className="h-px bg-cyber-red flex-1 opacity-50"/>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((proj, i) => (
               <motion.div key={proj._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                 className="bg-cyber-dark border border-white/10 rounded-xl overflow-hidden group hover:border-cyber-red/50 transition-all"
               >
                  <div className="h-48 bg-black/50 relative">
                    ...
                    <div className="absolute top-4 right-4 p-2 bg-black/80 rounded-full border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                        {proj.liveLink && <a href={proj.liveLink} target="_blank"><ExternalLink size={16} className="text-cyber-red"/></a>}
                    </div>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {proj.techStack.map((t, idx) => <span key={idx} className="px-2 py-1 bg-white/5 text-[10px] font-mono text-gray-300 rounded border border-white/5">{t}</span>)}
                    </div>
                  </div>
               </motion.div>
            ))}
            ...
            {certificates.map((cert) => (
               <motion.div key={cert._id} whileHover={{ y: -5 }} className="relative h-64 bg-gradient-to-br from-neutral-900 to-black border border-white/10 rounded-xl p-6 flex flex-col justify-between overflow-hidden group cursor-pointer">
                 ...
                 <div className="flex justify-between relative z-10">
                    <Shield className="text-cyber-red"/>
                 ...
                 <div className="flex justify-between items-center relative z-10">
                    <div className="flex gap-1"><div className="w-1 h-1 bg-cyber-red rounded-full"></div><div className="w-1 h-1 bg-cyber-red rounded-full"></div><div className="w-1 h-1 bg-cyber-red rounded-full"></div></div>
                    {cert.certUrl && <a href={cert.certUrl} target="_blank" className="text-cyber-red opacity-0 group-hover:opacity-100 transition-opacity"><ArrowRightCircle size={20}/></a>}
                 </div>
               </motion.div>
            ))}
         </div>
      ...
      <section id="contact" className="py-20 px-4 border-t border-white/5 bg-black/50 text-center">
          <Lock className="mx-auto text-cyber-red mb-6" size={48} />
          ...
          {profile.email && <a href={`mailto:${profile.email}`} className="inline-flex items-center gap-2 px-8 py-4 bg-cyber-red text-black font-bold font-mono rounded-lg hover:bg-red-500 transition-all"><Mail size={18}/> INITIATE EMAIL</a>}
      </section>
